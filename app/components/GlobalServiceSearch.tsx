...
export default function GlobalServiceSearch() {
    ...
    const handleSubmit = () => {
        ...
        router.push(`/${prefix}/${path.join("/")}`);  // ✅ prefix use karo
    };
        ...
        <div
            ...
            {showDropdown && filtered.length > 0 && (
                <ul className="absolute left-0 right-0 mt-2 w-full bg-black border border-white/10 rounded-2xl shadow-2xl max-h-60 overflow-y-auto z-50 text-start">
                    {filtered.map((service) => (
                        <li
                            key={`${service.prefix}-${service.fullPath.join("/")}`}
                            onClick={() => handleSelect(service)}
                            ...
                            {/* ✅ Badge — Events ya Solutions */}
                            <span className={`text-xs px-2 py-0.5 rounded-full ml-2 ${service.prefix === "solutions"
                                    ? "bg-blue-500/20 text-blue-400"
                                    : "bg-yellow-500/20 text-yellow-400"
                                }`}>
                                {service.prefix === "solutions" ? "Solution" : "Event"}
                            </span>
                        </li>
                    ))}
                </ul>
